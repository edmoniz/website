/*
  ...
  Scans every images-only gallery folder under Galleries/ (a folder with
  no .html file, just image files) and writes an images.json into it,
  built from the image files and their .xmp metadata. galleries.js
  ...
  images are added, removed, or retitled in an images-only gallery
  folder:
...
if (imagesOnlyEntries.length === 0) {
  console.log('No images-only gallery folders found under Galleries/.');
} else {
  imagesOnlyEntries.forEach(({ folder }) => {
    ...
    console.log(`Wrote ${data.images.length} image entr${data.images.length === 1 ? 'y' : 'ies'} to ${folder}/images.json`);
  });
}
